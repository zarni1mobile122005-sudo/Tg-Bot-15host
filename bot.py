...
# --- CONFIGURATION ---
API_TOKEN = '8329246254:AAFfdNv5dDG72Z0gwNyaQAwpq7ncsndysKg'
...
def get_dashboard_markup(uid):
    # Slot Buttons ကို ၅ ခုတစ်တန်း ပေါ်အောင် row_width=5 ထားထားပါတယ်
    markup = types.InlineKeyboardMarkup(row_width=5)
    
    slots_btns = []
    # ၁ ခုကနေ ၁၅ ခုအထိ Slot buttons များ တိုးမြှင့်လိုက်ခြင်း
    for i in range(1, 16):
        slot_str = str(i)
        is_running = False
        ...
        slots_btns.append(types.InlineKeyboardButton(f"{status_dot} S{i}", callback_data=f"select_slot_{i}"))
    ...
    # Control buttons များကို အောက်တစ်တန်းစီ ခွဲထုတ်ပေးခြင်း
    if action_btn:
...
def dashboard(message):
    ...
    dashboard_ui = (
        ...
        f"┣ Uritable: `{uptime}`\n"
        f"┗ Total Running Bots: `{active_count} / 15` 🔥\n\n" # UI မှာ 15 ထိ ပြောင်းပေးထားပါတယ်
        f"🖥 **SERVER ALLOCATION**\n"
...
def handle_text(message):
    ...
    if message.text in ['🖥 Dashboard', '⚙️ Control Center']:
        ...
        status_text = "📂 **Your Workspace Status (15 Slots):**\n\n"
        # Project စာရင်းပြသမှုကိုလည်း ၁၅ ခုစာအထိ ကွင်းဆက်ပတ်ပြီး ပြပေးထားပါတယ်
        for i in range(1, 16):
            slot_str = str(i)
            name = "Empty Slot"
